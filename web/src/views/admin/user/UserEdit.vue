...
const form = useForm({
  ...
  initialValues: { username: "", email: "", roleId: null, status: true, photo: "" },
});
...
const fetchUser = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      toast.add({ severity: "error", summary: t("crud.errorFetch", { module: labels.name }), life: 3000 });
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      toast.add({ severity: "error", summary: response.message || t("crud.errorUpdate", { module: labels.name }), life: 3000 });
    }
  } catch (error: any) {
...
onMounted(() => Promise.all([fetchRoles(), fetchUser()]));
</script>

<template>
  <Toast />
  <!-- Loading state -->
  <div v-if="fetchingData" class="flex flex-col items-center justify-center min-h-96 gap-4">
    <ProgressSpinner style="width:50px;height:50px" />
    <p class="text-surface-500 font-medium">{{ crud.fetchingData }}</p>
  </div>
  ...
  <div v-else class="space-y-4">
    ...
    <Card class="shadow-lg">
      ...
      <template #content>
        <form @submit="onSubmit" id="userEditForm" class="space-y-6">
      ...
      <template #footer>
        <div class="flex justify-end gap-2 border-t pt-4">
          ...
          <Button type="submit" form="userEditForm" :label="crud.save" :loading="submitting" />
        </div>
      </template>
    </Card>
  </div>
